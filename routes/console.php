...
Artisan::command('sync360:sync-replies {tenant? : Tenant ID (optional — all live tenants if omitted)}', function (?string $tenant = null) {
    /** @var \App\Services\WorkspaceSessionLogReader $reader */
    $reader = app(\App\Services\WorkspaceSessionLogReader::class);
    ...
    if ($tenant) {
        $query->where(function ($q) use ($tenant): void {
            $q->where('tenant_id', $tenant)->orWhere('id', $tenant);
        });
    }
    ...
    foreach ($tenants as $t) {
        ...
        $pendingLogs = \App\Models\ConversationLog::query()
        ...
        try {
            ...
            $errors++;
            Log::warning('sync360:sync-replies failed for tenant.', [
...
})->purpose('Backfill ConversationLog.message_out from OpenClaw workspace session logs');
