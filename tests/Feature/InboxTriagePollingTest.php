...
class InboxTriagePollingTest extends TestCase
{
        ...
        $messenger->shouldReceive('sendOperational')
            ...
            ->withArgs(function (Tenant $value, string $channel, string $from, string $message, array $requiredSkillIds): bool {
                return $value->slug === 'lead-shop'
                    ...
                    && str_contains($message, 'Critical Runtime Contracts for Telegram, Google Drive triage logging, Google Sheets lead logging, and analytics')
    ...
    public function test_messages_are_skipped_when_metadata_marks_them_sent_even_if_summary_labels_are_incomplete(): void
    {
        $tenant = $this->seedInboxTenant('metadata-sent-shop');
        $gmail = Mockery::mock(TenantInboxGmailRuntimeService::class);
        $messenger = Mockery::mock(TenantWorkspaceMessenger::class);

        $gmail->shouldReceive('searchRecentInbox')->once()->andReturn([[
            'id' => 'msg-metadata-sent-001',
            'thread_id' => 'thread-metadata-sent-001',
            'from' => 'Owner <metadata-sent-shop@gmail.test>',
            'subject' => 'Re: What services do you offer?',
            'labels' => [],
        ]]);
        $gmail->shouldReceive('getMessage')->once()->andReturn([
            'raw' => "id\tmsg-metadata-sent-001\nthread_id\tthread-metadata-sent-001\nlabel_ids\tSENT\nfrom\tOwner <metadata-sent-shop@gmail.test>\nsubject\tRe: What services do you offer?\n\nThanks for your message.",
            'metadata' => [
                'id' => 'msg-metadata-sent-001',
                'thread_id' => 'thread-metadata-sent-001',
                'label_ids' => 'SENT',
        ...
        $this->assertDatabaseHas('tenant_inbox_monitor_messages', [
            'tenant_id' => $tenant->id,
            'gmail_message_id' => 'msg-metadata-sent-001',
            'status' => TenantInboxMonitorMessage::STATUS_SKIPPED,
            'skip_reason' => 'noise_label:SENT',
        ]);
    }

    public function test_messages_from_connected_google_identity_are_skipped_without_invoking_gateway(): void
    {
        $tenant = $this->seedInboxTenant('self-sender-shop');
        $gmail = Mockery::mock(TenantInboxGmailRuntimeService::class);
        $messenger = Mockery::mock(TenantWorkspaceMessenger::class);

        $gmail->shouldReceive('searchRecentInbox')->once()->andReturn([[
            'id' => 'msg-self-sender-001',
            'thread_id' => 'thread-self-sender-001',
            'from' => 'self-sender-shop@gmail.test',
            'subject' => 'Re: What services do you offer?',
            'labels' => ['INBOX'],
        ]]);
        $gmail->shouldReceive('getMessage')->once()->andReturn([
            'raw' => "id\tmsg-self-sender-001\nthread_id\tthread-self-sender-001\nlabel_ids\tUNREAD,INBOX\nfrom\tself-sender-shop@gmail.test\nsubject\tRe: What services do you offer?\n\nThanks for your message.",
            'metadata' => [
                'id' => 'msg-self-sender-001',
                'thread_id' => 'thread-self-sender-001',
                'label_ids' => 'UNREAD,INBOX',
    ...
    public function test_expired_tenant_with_runtime_reply_override_receives_allowed_reply_policy_context(): void
    {
        $tenant = $this->seedInboxTenant('expired-replies-allowed-shop', tenantOverrides: [
            'trial_status' => TrialStatus::Expired,
            'allow_polling_when_trial_expired' => true,
        ...
        $gmail->shouldReceive('searchRecentInbox')->once()->andReturn([[
            'id' => 'msg-expired-allowed-001',
            'thread_id' => 'thread-expired-allowed-001',
            'from' => 'Jane Buyer <jane@example.com>',
            'subject' => 'Need a quote',
            'labels' => ['INBOX'],
        ]]);
        $gmail->shouldReceive('getMessage')->once()->andReturn([
            'raw' => "id\tmsg-expired-allowed-001\nthread_id\tthread-expired-allowed-001\nlabel_ids\tINBOX\nfrom\tJane Buyer <jane@example.com>\nsubject\tNeed a quote\n\nPlease quote this project.",
            'metadata' => [
                'id' => 'msg-expired-allowed-001',
                'thread_id' => 'thread-expired-allowed-001',
                'label_ids' => 'INBOX',
    ...
    private function seedInboxTenant(string $slug, bool $googleVerified = true, array $tenantOverrides = []): Tenant
    {
        ...
        $version = SkillCatalogVersion::query()->firstOrCreate(
            ['skill_catalog_item_id' => $item->id, 'version' => '1.6.2'],
            [
                'skill_key' => 'inbox-triage',
