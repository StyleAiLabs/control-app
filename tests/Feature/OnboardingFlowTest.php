...
class OnboardingFlowTest extends TestCase
{
    ...
    public function test_go_live_includes_owner_google_workspace_guidance_when_connected(): void
    {
        ...
        $this->assertStringContainsString('Google Sheets qualified lead row', File::get($localRuntimePath.'/.openclaw/workspace/skills/inbox-triage/SKILL.md'));
        $this->assertStringContainsString('Sync360 Inbox Triage Qualified Leads', File::get($localRuntimePath.'/.openclaw/workspace/skills/inbox-triage/SKILL.md'));
        ...
        $this->assertStringContainsString('For qualified leads, emit analytics immediately after classification and stable lead-id construction, before Telegram, Google Drive, or Google Sheets side effects.', File::get($localRuntimePath.'/.openclaw/workspace/skills/inbox-triage/SKILL.md'));
        $this->assertStringContainsString('If runtime budget, token, or model availability becomes constrained mid-run, skip non-essential narration and execute the analytics helper before any remaining optional side effects.', File::get($localRuntimePath.'/.openclaw/workspace/skills/inbox-triage/SKILL.md'));
        ...
        $this->assertStringContainsString('Match the tenant communication style captured during onboarding.', File::get($localRuntimePath.'/.openclaw/workspace/skills/inbox-triage/SKILL.md'));
        $this->assertStringContainsString('Do not include literal escape sequences such as `\\n`, `\\r`, or `\\t` in the reply body.', File::get($localRuntimePath.'/.openclaw/workspace/skills/inbox-triage/SKILL.md'));
        $this->assertStringContainsString('Keep the reply body ASCII-only and avoid decorative symbols, emoji, markdown formatting, smart quotes, bullets, or other special characters', File::get($localRuntimePath.'/.openclaw/workspace/skills/inbox-triage/SKILL.md'));
        ...
        $this->assertStringContainsString('gog gmail send --reply-to-message-id <gmail_message_id> --subject \'<subject>\' --body \'<plain-text-body>\'', File::get($localRuntimePath.'/.openclaw/workspace/skills/inbox-triage/SKILL.md'));
        $this->assertStringContainsString('Do not combine `--reply-to-message-id` with `--thread-id` in the standard Inbox Triage reply flow.', File::get($localRuntimePath.'/.openclaw/workspace/skills/inbox-triage/SKILL.md'));
        $this->assertStringNotContainsString('gog gmail send --reply-to-message-id <gmail_message_id> --subject \'<subject>\' --body \'<plain-text-body>\' --thread-id <gmail_thread_id>', File::get($localRuntimePath.'/.openclaw/workspace/skills/inbox-triage/SKILL.md'));
        ...
        $this->assertStringContainsString('Verified Gmail direct-reply surface: `gog gmail send --reply-to-message-id <gmail_message_id> --subject "<subject>" --body "<plain-text-body>"`.', File::get($localRuntimePath.'/.openclaw/workspace/TOOLS.md'));
        $this->assertStringContainsString('do not combine `--reply-to-message-id` with `--thread-id` in the standard direct reply flow', strtolower(File::get($localRuntimePath.'/.openclaw/workspace/TOOLS.md')));
