...
- Channel-step navigation model: Step 5 now always renders an explicit `Continue To Google Workspace` button, even if the channel is already connected or the customer wants to skip ahead and come back later, so the wizard never traps them on the channel panel
