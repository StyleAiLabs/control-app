...
**2026-05-03 Customer conversations surface removal:** the authenticated customer portal no longer includes a first-class `Conversations` page. `routes/web.php` no longer registers `/conversations` for customer workspace access, and the former customer `ConversationsController` plus `resources/views/conversations/index.blade.php` surface have been removed. This is an access/presentation boundary change only: the `conversation_logs` data model, session-log sync command, and AI summary generation pipeline still exist for internal Sync360 workflows and operator-facing evidence needs.
**2026-05-03 Tenant-scoped control-plane LiteLLM calls:** customer-triggered control-plane helper calls no longer borrow the platform virtual key. `BusinessExtractionService` now resolves LiteLLM credentials through `TenantRuntimeCapabilityService::resolveRuntimeCredentials()`, so onboarding website extraction and onboarding-generated workspace files use the tenant virtual key by default and inherit any explicit tenant runtime API-key override. `ConversationSummaryService` now follows that same tenant-scoped resolver for session summaries, and it fails closed when the tenant has no usable runtime credential instead of silently falling back to `services.litellm.virtual_key`. Legitimate platform/master-key paths remain the LiteLLM admin/service operations such as key generate/update/delete/info and spend-log import.
**2026-05-01 LiteLLM runtime cost attribution hardening:** `LiteLlmRuntimeCostSyncService` now normalizes more live `/spend/logs` payload variants before tenant resolution and reporting. Tenant attribution no longer depends only on `metadata.tenant_id` and top-level alias fields; the sync also accepts nested metadata tenant/id alias variants plus snake_case timestamp fields, and it persists the resolved tenant key alias when the live row omits a top-level alias. The goal is to keep `tenant_runtime_usage_events` usable for tenant-scoped cost observability even when LiteLLM returns slightly different field shapes than the original test fixture.
**2026-05-01 Inbox monitor customer-only filtering hardening:** `TenantInboxMessageFilter` now acts as a stricter pre-dispatch gate for inbox-triage polling. Beyond Gmail category labels, self-sender checks, and a small fixed subject denylist, the filter now also rejects bulk/list metadata markers and marketing-broadcast patterns inferred from subject/body/sender signals. The intent is to keep the runtime-facing inbox monitor focused on plausible customer or business enquiries instead of general newsletters, community digests, and product-announcement mail.
**2026-04-30 Expired-trial inbox policy evidence surfaces:** Sync360 now derives expired-trial inbox policy presentation through a shared `ExpiredTrialInboxPolicySurface` service instead of leaving admin and customer wording to separate ad hoc controller branches. Admin uses that shared state to surface exact expired-trial override posture (`fully paused`, `monitoring only`, `fully allowed`, or custom mix) alongside the raw polling/reply/LiteLLM flags. Customer-facing surfaces deliberately compress the same policy into two states: when all three expired-trial overrides are enabled, dashboard inbox/assistant cards continue to use the normal live derivation; otherwise expired-trial customers are shown a forced paused state that says inbox monitoring, customer replies, and live assistant work are not happening until access is restored.
**2026-05-01 Inbox-triage quote follow-up routing hardening:** the owner follow-up path for `Lead ref` notifications now has an explicit prompt-layer contract that quote or estimate requests, including inline owner pricing, must route through the assigned `pdf-generation` skill before any customer-facing quote email is sent. This rule now exists in both the shipped skill packs and the composed tenant heartbeat guidance so the runtime does not treat a plain-text Gmail quote as an acceptable fallback from inbox-triage.
**2026-04-30 Expired-trial inbox policy evidence surfaces:** Sync360 now derives expired-trial inbox policy presentation through a shared `ExpiredTrialInboxPolicySurface` service instead of leaving admin and customer wording to separate ad hoc controller branches. Admin uses that shared state to surface exact expired-trial override posture (`fully paused`, `monitoring only`, `fully allowed`, or custom mix) alongside the raw polling/reply/LiteLLM flags. Customer-facing surfaces deliberately compress the same policy into two states: when all three expired-trial overrides are enabled, dashboard inbox/assistant cards continue to use the normal live derivation; otherwise expired-trial customers are shown a forced paused state that says inbox monitoring, customer replies, and live assistant work are not happening until access is restored.
**2026-04-30 Billing downgrade/cancellation entitlement reconciliation:** Stripe subscription lifecycle reconciliation now treats the subscribed Stripe price as the plan source of truth during `customer.subscription.updated`, so customer-portal plan changes can move a tenant between Sync360 billing plans even when Stripe metadata is stale. Billing entitlement sync is now scoped only to the billing-managed included-skill universe across configured plans, which allows Sync360 to add or revoke plan-owned modules on upgrade/downgrade without disabling unrelated manual or onboarding-driven assignments. When Stripe later emits `customer.subscription.deleted`, Sync360 now clears the stored `billing_plan` and billing-cycle timestamps while suspending the LiteLLM budget, preventing cancelled tenants from continuing to look like they are on an old paid plan.
**2026-04-30 Expired-tenant AI-runtime hold for inbox triage:** Sync360 now distinguishes three expired-tenant controls for pre-paid tenants: inbox polling visibility, customer-facing reply permission, and AI-runtime execution permission. `CommercialAccessPolicy::canPollInbox()` may remain true when `allow_polling_when_trial_expired` is enabled, but runtime dispatch now separately requires `allow_litellm_when_trial_expired` through `canDispatchAiRuntimeWork()`. As a result, `TenantInboxTriagePollingService` can continue to observe and persist candidate Gmail events while skipping runtime delivery with `commercial_hold:ai_runtime_paused` when LiteLLM-backed execution is still on hold.
**2026-04-30 Expired-trial inbox-triage reply safety context:** Sync360's internal Gmail inbox trigger now passes explicit delivery-policy context into inbox-triage so the runtime can distinguish `polling allowed` from `customer-facing Gmail replies allowed`. When expired-trial polling remains enabled but customer-facing runtime replies are paused, `TenantInboxTriagePollingService` tells the skill not to send a Gmail reply or create a draft in that run while still allowing non-reply workflow steps such as classification, operator notification, Google Drive logging, Google Sheets logging, and analytics.
**2026-04-29 Runtime cost sync windowing and response-shape compatibility:** `LiteLlmRuntimeCostSyncService` no longer assumes LiteLLM `/spend/logs` returns a small `{ data: [...] }` payload for the full lookback period. The service now requests one day at a time using matching `start_date` / `end_date` bounds, which keeps each HTTP response small enough for the default PHP memory limit, and it accepts both top-level arrays and `{ data: [...] }` envelopes before normalizing usage rows into `tenant_runtime_usage_events`.
...
**2026-04-28 Tenant apply/rollout diagnostic payload hardening:** `TenantAgentCustomizationApply.composed_output_json` remains the audit/preview snapshot for composed runtime output, but it must never store raw binary workspace asset bytes. `ComposedTenantRuntime::diagnosticPayload()` now preserves text workspace file contents while summarizing binary files such as `business-assets/logo.png` into metadata (`kind`, `size_bytes`, `sha256`) so tenant apply and skill rollout jobs stay JSON-serializable even when the workspace includes uploaded business assets.
...
**2026-04-27 Admin paused-agent presentation fix:** the admin tenant list and tenant-detail status strip must distinguish runtime health from commercial reply availability. `Workspace` continues to reflect whether the tenant container is running, but `Agent` now resolves through expired-trial policy so an expired tenant with blocked runtime replies presents as `paused` instead of `live`, even if the workspace container itself remains healthy.
**2026-04-27 Inbox poller run idempotency hardening:** `TenantInboxTriagePollingService` now treats `tenant_inbox_monitor_messages` as the dispatch ownership boundary, not just a log table. Before calling the tenant workspace hook, the poller atomically flips the Gmail message row into a transient `dispatching` state and increments attempts; concurrent pollers skip rows already being dispatched unless the lease has gone stale, which prevents the same inbound Gmail message from waking multiple agent runs during polling races or repeated Gmail fetches.
**2026-04-27 Conversation-log schema drift detection:** Sync360 now treats the control-plane `conversation_logs` schema as a runtime prerequisite instead of assuming migrations are current. A shared schema guard validates required columns before tenant provisioning, tenant health checks, conversation sync, and customer-facing runtime wakeups; when drift is detected, Sync360 fail-closes those paths with an explicit remediation message telling operators to run `php artisan migrate` and then retry the blocked action or rerun health checks.

**2026-04-23 DaisyUI Design System PoC:** daisyUI now runs behind Sync360 Blade wrappers rather than appearing as public view markup. The Tailwind entrypoint registers daisyUI with the `dui-` prefix and a custom `sync360` theme, while reusable wrappers live in `resources/views/components/ui/*` and marketing wrappers live in `resources/views/components/marketing/*`. The PoC intentionally covers the landing page, admin tenant list, and the full admin tenant detail tab set without changing controllers, routes, or tenant data shapes. Dense admin tables use fit-to-page layouts with truncation for long secondary strings, and tenant-detail tabs use fixed status icons plus quiet hover states. `DM Sans` remains the primary UI/status font; `JetBrains Mono` is limited to precise technical values, logs, hashes, and raw runtime output.

**2026-04-24 Admin Tenants scan-first refactor:** the admin tenant list now favors scan hierarchy over pure column count. The page adds a lightweight summary strip above the table, collapses the list into seven columns (`Tenant`, `Runtime`, `Provisioning`, `Google`, `Agent`, `Trial`, `Health`), groups workspace URL under runtime placement, and renders Google plus health/workspace as icon + label state rows instead of stacked badges. Primary tenant identity stays readable before secondary strings are truncated, and row-open behavior is signaled with a subtle chevron affordance. A same-day density pass also reduced summary-strip visual weight and clamped secondary note lines so the table stays calmer under real tenant data.

**2026-04-24 Tenant detail overview ownership pass:** the admin tenant detail surface now treats Overview as the canonical shared summary tab. The header uses a compact status strip instead of a full badge rail, tab styling lives in `resources/css/app.css` rather than page-local inline CSS, Workspace focuses on placement/profile context, and Google is structured around current state, recommended next step, and recent evidence instead of one undifferentiated metadata grid. Human-scanned status words and timestamps stay in `DM Sans`; technical mono remains for exact runtime strings.
...
**2026-04-25 Inbox Triage operator monitor and reply contract:** tenant admin now includes an `Inbox Monitor` tab on the existing tenant-detail route. The controller loads `tenant_inbox_monitor_states`, recent `tenant_inbox_monitor_messages`, Google runtime state, and the assigned `inbox-triage` version into one operator summary so support/debugging stays on the tenant detail surface instead of a separate admin page. Inbox Triage itself now has an explicit low-risk Gmail reply branch for basic support and business-information enquiries, and the runtime guidance uses the verified live `gog` Gmail write surface: `gog gmail send --reply-to-message-id <gmail_message_id> ...` for direct replies and `gog gmail drafts create --reply-to-message-id <gmail_message_id> ...` for draft-only flows.
**2026-04-25 Inbox Triage reply execution hardening:** the workspace-managed `inbox-triage` skill now treats basic-enquiry replies as a required executed side effect, not a future intention. For Sync360-triggered inbox events, low-risk basic enquiries must either run `gog gmail send --reply-to-message-id ...` immediately or explicitly fail/draft with matching `reply_status` evidence; summaries that promise a later clarifying email without a Gmail tool result are now considered invalid workflow completion. The generated tenant `HEARTBEAT.md` mirrors that rule so the runtime sees it even before opening the full skill file. The same hardening pass also reiterates that service/hours/coverage/basic-support questions stay out of the high-value Telegram path unless the message independently shows genuine commercial buying intent. A later copy-contract refinement also requires those Gmail auto-replies to follow the tenant tone chosen during onboarding and to use clean plain-text bodies without literal escape sequences or decorative special characters by default.
**2026-04-26 Shared GOG command contracts:** Sync360 now treats `App\Services\GogCommandCatalogService` as the canonical source for Inbox-Triage-relevant `gog` command shapes. Workspace-managed skills and generated tenant guidance should derive Gmail, Drive, and Sheets command wording from that shared contract layer instead of re-authoring raw command snippets independently. Upstream `gogcli` README usage is the primary reference, while live tenant evidence is used only to narrow Sync360's approved default shapes when the runtime proves stricter.
**2026-04-27 WeasyPrint capability rollback:** the short-lived `weasyprint` host-managed runtime capability has been intentionally reverted after live verification on the tenant VPS showed that mounting a host-built Python virtualenv into the OpenClaw container is not a dependable v1 capability boundary. The host install succeeded, but the container resolved a different Python minor version and failed with `ModuleNotFoundError`, so Sync360 has returned to the simpler `gog`-only capability surface until a container-aware PDF runtime approach is designed.
**2026-04-26 Tenant runtime model and API key overrides:** the existing admin `Agent Runtime` tab is now the control-plane source of truth for tenant-specific runtime model overrides and tenant runtime API key overrides. Model stays in `agent_defaults_json`; the sensitive API key lives in encrypted `tenant_agent_customizations` columns and is surfaced only as masked state in the UI. The agent-runtime apply path now updates runtime workspace files, `openclaw.json`, `.env`, and `compose.yaml`, then reloads the tenant runtime so credential/model changes become live through the same apply/revert lifecycle as prompt overrides. Provisioning and later compose regeneration also consult the tenant override key before falling back to `litellm_virtual_key`, while `OPENAI_BASE_URL` remains platform-managed.
**2026-04-25 Trial extension budget recovery:** the admin `extend trial` action now coordinates with LiteLLM budget state instead of only moving `trial_ends_at`. If a tenant has an existing LiteLLM key and persisted spend/budget data indicates exhausted AI credit headroom, the controller reuses the same LiteLLM key and calls the key-update path with a new ceiling of `max(spend, current max budget) + 5`. Time-only extensions do not touch LiteLLM budget when headroom remains.
...
**2026-04-27 Customer dashboard analytics windowing:** dashboard analytics are no longer hardcoded to a single 30-day slice. `DashboardController` now resolves a shared analytics window (`7d`, `30d`, `90d`, `ytd`) from the request, uses that period for skill analytics, reviewed inbox counts, inbox value-line copy, and the performance-series chart, and passes the same selection to all dashboard analytics panels so customer-visible KPI cards and charts stay in sync.
...
**2026-04-27 Customer profile surface refresh:** the customer profile page now composes the same daisyUI-backed Sync360 wrappers used by the newer dashboard/admin surfaces instead of the older raw `topbar` + two-panel layout. `ProfileController` now also passes normalized dependency-health data from `TenantWorkspaceDependencyHealthService` into the view so the profile page can keep Google Workspace / inbox-monitoring problems visible as customer-facing sync-readiness information without exposing low-level runtime internals. The rendered page uses a health rail plus grouped profile sections for editing, while dependency alerts become a focused sidebar action card rather than inline warning prose scattered through the form. It also now includes async business-logo upload, preview, replace, and remove behavior without a full page refresh, presented as a compact inline field inside `Identity and positioning` rather than a standalone feature panel.
...
Sync360's repo workflow for human-plus-agent delivery lives in [`artifacts/AGENTIC_DEVELOPMENT.md`](AGENTIC_DEVELOPMENT.md), with the portable version in [`artifacts/PORTABLE_AGENTIC_PLAYBOOK.md`](PORTABLE_AGENTIC_PLAYBOOK.md). The key architectural rule is that the board is the live execution surface and docs are bootstrap/support artifacts: initiative cards define intent, slice cards are the decision-complete execution unit, and the canonical templates under `templates/agentic-development/` exist so fresh sessions can re-enter work without relying on long chat transcripts. This workflow does not change runtime behavior directly, but it is part of the repo's control-plane operating contract because many implementation invariants, rollout checks, and canonical-doc obligations are now expected to be captured at the card level before code changes begin.
...
- Python virtualenv capabilities are not part of the supported runtime model right now; live WeasyPrint testing showed that host-vs-container interpreter drift makes mounted venvs unreliable unless Sync360 also owns the container runtime image or another container-aware dependency layer
...
- Catalog-managed skill packs:
  ...
  - use `resources/skill-packs/SYNC360-SKILL-FRAMEWORK.md` as the canonical external-skill framework and authoring prompt; it now covers package structure, runtime-model classification, module-local script/library rules, analytics emission, tracking payloads, privacy rules, versioning, release notes, rollout expectations, and runtime materialization standards
  - remote teams should also receive the nested starter pack under `resources/skill-packs/examples/starter-skill-module/` and the delivery gate in `resources/skill-packs/SYNC360-DEV-AGENT-HANDOFF-CHECKLIST.md`; the starter pack is intentionally nested so repo skill scans do not treat it as a real importable catalog item
...
10. Sync360 does not send Telegram notifications, does not classify lead quality, and does not reinterpret the agent response. The `inbox-triage` skill remains responsible for category, lead quality, Telegram notification, Google Drive logging, Google Sheets qualified-lead rows, and analytics. Its skill contract specifies each required side effect with idempotency, command/tool guidance, validation, and failure handling so the agent does not silently skip operational logs. For Gmail-triggered events, the Gmail message id is the primary lead id for Telegram `Lead ref`, Drive log naming, Sheets idempotency, and analytics conversion ids. High-value Telegram notifications include a visible exact `Lead ref: <gmail_message_id>` line so owner follow-up replies can reopen the exact Gmail message rather than reconstructing it from summary text. The current contract front-loads a strict Telegram `message` send schema, forbids poll fields on normal sends, enforces a high-value-only Telegram gate, gives concrete commercial quote/request-for-service high-intent signals, uses minimal `gog drive upload <localPath>` Drive logging, writes qualified leads to an auto-managed `Sync360 Inbox Triage Qualified Leads` spreadsheet / `Qualified Leads` tab, forbids `apply_patch` or local-only file edits as a substitute for Drive logging, forbids public web research for internal inbox triggers unless the owner asks, and requires analytics to be attempted independently for qualified leads with the manifest-required `event_id` / outcome payload fields even when Telegram, Drive, or Sheets logging fails.
...
3. writes `IDENTITY.md`, `SOUL.md`, `USER.md`, `BOOTSTRAP.md`, `TOOLS.md`, `PROFILE.md`, `HEARTBEAT.md`, and `BUSINESS_PROFILE.json` into `.openclaw/workspace/`, adds optional tenant-scoped business assets such as `business-assets/logo.<ext>`, and materializes workspace skill files under `.openclaw/workspace/skills/<skill-id>/...`
4. syncs only workspace artifacts with `syncWorkspaceFiles()`
...
- Gmail inbox polling must operate on message ids, not thread summaries. Gmail thread search can return a thread because some message in it is still in the inbox while the newest message on that thread is Sync360's own `SENT` auto-reply; using thread-level search caused follow-up customer replies to be missed and outbound replies to be re-ingested. Sync360 now uses `gog gmail messages search` plus a `-label:sent -label:draft` guard and also skips `SENT` / `DRAFT` labels in the inbox filter as defense in depth.
- Gmail inbox filtering must trust the fetched message metadata over incomplete search-summary labels. The poller should merge summary labels with `gog gmail get` metadata labels before deciding whether to skip a message, and it should also skip messages whose sender email equals the tenant’s connected Google Workspace identity. This prevents the tenant’s own outbound Gmail copy from being re-routed into inbox-triage even when Gmail search summaries are missing `SENT`.
...
- Custom workspace skill activation now has an explicit runtime contract layered on top of assignment state. Sync360 writes `.openclaw/workspace/.sync360/runtime-skill-contract.json` with the canonical expected skill set, a deterministic `skill_set_hash`, the last verified live runtime skill list, and the latest verification error.
- The current permanent fix for stale runtime activation is session rotation plus verification, not prompt wording alone. When the expected skill set changes, or when a tenant runtime model/API-key override changes, Sync360 clears the tenant's OpenClaw agent session state under `data/agents`, reloads the runtime, and then verifies `openclaw skills list --eligible` before declaring apply/go-live success.
- Runtime refresh is no longer just a diagnostic button. Admin runtime refresh, tenant apply/go-live, and custom-skill trigger delivery now converge on `TenantRuntimeSkillActivationService`, which reuses the same verification path and persists the result back into the runtime skill contract artifact.
- Trigger delivery is now fail-closed for custom skills that depend on runtime readiness. `TenantWorkspaceMessenger` can require specific skill ids, verifies them against the current contract, performs one self-heal cycle when needed, and refuses delivery if the required skill still is not active.
- `SENT_TO_AGENT` remains a delivery-state label, not a “reply sent” label, but it now means Sync360 successfully delivered to a runtime whose required skill contract had been verified for that trigger.
- The runtime verification path must prefer JSON skill discovery output over human-formatted table output. Current OpenClaw VPS builds can render `openclaw skills list --eligible` as a Unicode table, so Sync360 should use `--eligible --json` or `--json` first and treat text parsing as fallback only.
- Runtime skill discovery must canonicalize workspace display labels before contract comparison. Some OpenClaw JSON payloads expose workspace skills under `name` as human labels like `Inbox Triage` instead of the canonical skill id, so Sync360 normalizes discovered skill names back to stable kebab-case ids before checking `expected_skill_ids` and `required_skill_ids`.
...
- Expired-trial enforcement now has two layers for direct customer channels. Suspending the tenant LiteLLM key stops model generation, but OpenClaw can still poll Telegram and surface provider errors back to customers unless the runtime channel is paused too. Sync360 therefore keeps the saved Telegram setup in `tenant.channel_config`, but removes `channels.telegram` from live `openclaw.json` whenever `ExpiredTrialAccessPolicy` says direct customer-facing runtime work is not allowed, and restores that channel config through `TenantAgentSyncService::syncSavedChannelIfReady()` when the tenant is reactivated or an admin enables the runtime-replies override.
- Expired-trial Gmail inbox polling and internal skill dispatch are intentionally separate from customer-facing runtime replies. Sync360 can keep inbox-triage trigger delivery active for expired tenants through `TenantWorkspaceMessenger::sendOperational()` when `allow_polling_when_trial_expired` is enabled, while still blocking direct customer reply traffic unless `allow_runtime_replies_when_trial_expired` is also enabled.
- `conversation_logs` schema drift is a control-plane reporting concern, not a prerequisite for tenant provisioning or runtime readiness checks. Provisioning and tenant health checks must continue to run when reporting columns drift, while reply sync and any code path that directly reads or writes `conversation_logs` remain fail-closed behind `ConversationLogSchema`.
