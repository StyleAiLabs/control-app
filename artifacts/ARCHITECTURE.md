...
- The current permanent fix for stale custom-skill activation is session rotation plus verification, not prompt wording alone. When the expected skill set changes, Sync360 clears the tenant's OpenClaw agent session state under `data/agents`, reloads the runtime, and then verifies `openclaw skills list --eligible` before declaring apply/go-live success.
