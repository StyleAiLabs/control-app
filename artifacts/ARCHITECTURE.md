...
8. The trigger message is neutral: it says the message is business-plausible, explicitly says Sync360 has not classified it as high-value, includes optional `telegram_default_chat_id` context, tells the agent to route the event to the workspace skill file at `./skills/inbox-triage/SKILL.md`, not `/app/skills`, and explicitly frames the hook as an internal operating task that must execute the inbox-triage workflow now rather than returning only an assessment or plan.
...
10. Sync360 does not send Telegram notifications, does not classify lead quality, and does not reinterpret the agent response. The `inbox-triage` skill remains responsible for category, lead quality, Telegram notification, Google Drive logging, and analytics. Its skill contract specifies each required side effect with idempotency, command/tool guidance, validation, and failure handling so the agent does not silently skip operational logs. High-value Telegram notifications include a visible `Lead ref: <gmail_message_id>` so owner follow-up replies can reopen the exact Gmail message rather than reconstructing it from summary text. The current contract uses a strict Telegram `message` send schema, forbids poll fields on normal sends, uses minimal `gog drive upload <localPath>` Drive logging, forbids public web research for internal inbox triggers unless the owner asks, and requires analytics to be attempted independently for qualified leads even when Telegram or Drive logging fails.
