...
**2026-04-28 Admin cost observability surfaces:** admin runtime-cost reporting now follows the existing calm analytics pattern rather than inventing a new dashboard language. The fleet `Cost Observability` page reuses the same topbar + four-stat summary + paired panel grid rhythm as `Skill Analytics`, while the tenant `Costs` tab follows the standard tenant-detail panel stack: one summary panel, one two-column breakdown row, then a recent-events list. Cost, request, and token metrics should read as operational evidence, not billing UI, so use compact stat cards, `badge` totals for row-level cost callouts, and short explanatory copy. Keep unmatched runtime usage visible as regular metadata rows under `Recent Unmatched Usage` rather than escalating it into an alert banner by default.
**2026-04-30 Expired-trial inbox policy presentation:** customer and admin surfaces now intentionally diverge in complexity. Customer-facing expired-trial messaging should stay binary: either everything continues to feel normal because all expired-trial overrides are enabled, or the assistant/inbox experience is shown as paused while expired with plain-language copy that no inbox reads or replies are happening until access is restored. Admin surfaces should stay exact and expose the real override posture with explicit labels such as `Fully paused while expired`, `Monitoring only while expired`, and `Fully allowed while expired`, plus the individual polling/reply/LiteLLM flags. Do not leak the admin-only mixed states into customer-facing copy.
...
**2026-04-28 Customer workspace-content surface:** the new `Workspace Content` page should feel like a calm “teach the assistant what to rely on” journey, not a document-management backend. Use the newer daisyUI-backed Sync360 wrappers, a four-card health rail plus one clear inline status band, grouped sections for `Business facts`, `Quick answers`, and `Documents`, then a separate full-width `Website content` section for multi-site review work. Support up to five website cards, keep the onboarding/profile website visible as the default seeded entry until it is reviewed, and reveal the add-site composer only on demand. Avoid dense admin-style tables as the default customer treatment; favor warm expandable cards, supportive summaries, and one primary action at a time. Do not expose workspace file paths, machine-readable contract filenames, or other implementation details in the customer UI.
...
- the customer `Workspace Content` page should stay separate from `Profile` and focus on approved assistant knowledge: quick-answer text, uploaded documents, and review-gated website snapshots
- on `Workspace Content`, use expandable cards and summary counts instead of customer-facing tables, keep async upload/review feedback short and inline, and treat the website workflow as its own full-width section rather than squeezing multi-site review into the sidebar
