...
- When the owner says `Generate a quote`, `Send a quote`, `Quote this`, asks you to reply with a quote, provides quote pricing, or otherwise instructs you to send quote/estimate pricing after extracting the email content, invoke the **pdf-generation** skill with `document_type: quote`. Pass the extracted customer name, email domain, job scope, and the Gmail message ID as `source_reference`. Read `skills/pdf-generation/SKILL.md` and follow it exactly — do not use your default document generation behavior.
- In this quote-follow-up branch, owner-provided prices or line items are inputs to **pdf-generation**, not permission to send a plain-text Gmail quote reply directly from inbox-triage.
